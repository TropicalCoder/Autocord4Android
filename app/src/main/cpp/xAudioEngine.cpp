...
int AudioEngine::ProcessData(bool bCalledFromCode)
{
 ...
 pFFTManager->GetBinsForFrequencyRange(fTriggerFreqMin, fTriggerFreqMax, &loBin, &hiBin);

 // Produce the data that the graph depends on
 char *pMusicalNote = pMusicalGraph->FindPeaks(pFFTManager, loBin, hiBin);
 ...
 if(curMode == RECORD)
 {
    int retval = HandleRecord(pMusicalGraph->fPeakFreq, pMusicalGraph->fPeakDBs);
    if(retval > 0)
    {
 ...
 // Monitor mode
 bool bTriggered = (pMusicalGraph->fPeakDBs > fTriggerDBs) && (pMusicalGraph->fPeakFreq >= fTriggerFreqMin) && (pMusicalGraph->fPeakFreq <= fTriggerFreqMax);
 if(bTriggered)
 {
     if(pMusicalNote != nullptr)
     {
...
float AudioEngine::GetGainApplied()
{
    ...
    // For 60 dB of gain, signal is multiplied by 1000
    // For 40 dB of gain, signal is multiplied by 100
    // For 0 dB of gain,  signal is multiplied by 1
    // For negative gain, (in other words, attenuation)
    // signal is multiplied by < 1.0, but we don't do that.
    // Therefore, if fLocalPeakAmplitude < 1.0 it is some kind of error.
